...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning, bool mcts);
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning, bool mcts) {
        ...
        &&  pos.non_pawn_material(pos.side_to_move()))
    {
        ...
        Value nullValue = depth-R < ONE_PLY ? -qsearch<NonPV, false>(pos, ss+1, -beta, -beta+1)
                                            : - search<NonPV>(pos, ss+1, -beta, -beta+1, depth-R, !cutNode, true, mcts);
        pos.undo_null_move();
        ...
        if (nullValue >= beta)
        {
            ...
            Value v = depth-R < ONE_PLY ? qsearch<NonPV, false>(pos, ss, beta-1, beta)
                                        :  search<NonPV>(pos, ss, beta-1, beta, depth-R, false, true, mcts);
        ...
        &&  abs(beta) < VALUE_MATE_IN_MAX_PLY)
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (pos.legal(move))
            {
                ...
                pos.do_move(move, st);
                value = -search<NonPV>(pos, ss+1, -rbeta, -rbeta+1, depth - 4 * ONE_PLY, !cutNode, false, mcts);
                pos.undo_move(move);
                if (value >= rbeta)
                    return value;
            }
        ...
        && (PvNode || ss->staticEval + 256 >= beta))
    {
        ...
        search<NT>(pos, ss, alpha, beta, d, cutNode, true, mcts);
...
moves_loop: // When in check search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
          ...
          &&  pos.legal(move))
      {
          ...
          value = search<NonPV>(pos, ss, rBeta - 1, rBeta, d, cutNode, true, mcts);
          ...
          && (!captureOrPromotion || moveCountPruning))
      {
          ...
          value = -search<NonPV>(pos, ss+1, -(alpha+1), -alpha, d, true, false, mcts);
      ...
      if (doFullDepthSearch)
          value = newDepth <   ONE_PLY ?
                            givesCheck ? -qsearch<NonPV,  true>(pos, ss+1, -(alpha+1), -alpha)
                                       ...
                                       : - search<NonPV>(pos, ss+1, -(alpha+1), -alpha, newDepth, !cutNode, false, mcts);
      ...
      if (PvNode && (moveCount == 1 || (value > alpha && (rootNode || value < beta))))
      {
          ...
          value = newDepth <   ONE_PLY ?
                            givesCheck ? -qsearch<PV,  true>(pos, ss+1, -beta, -alpha)
                                       ...
                                       : - search<PV>(pos, ss+1, -beta, -alpha, newDepth, false, false, mcts);
      }
...
//    Threads.stopOnPonderhit = Threads.stop = false;
    ...
    Value value = depth <   ONE_PLY ?
                     pos.checkers() ? qsearch<PV,  true>(pos, ss, alpha, beta)
                                    ...
                                    :  search<PV>(pos, ss, alpha, beta, depth, false, false, true);
